  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-sm font-bold">
      ...
      {showForm && (
        <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-5 space-y-4">
          ...
          {(["physical", "technical", "tactical", "attitudinal"] as const).map((k) => (
            <div key={k}>
              <label className="text-xs text-slate-400 capitalize">{k === "physical" ? "Físico" : k === "technical" ? "Técnico" : k === "tactical" ? "Táctico" : "Actitudinal"}</label>
          ...
          />
          <button onClick={save} className="px-5 py-2 rounded-lg bg-amber-400 text-slate-900 font-bold text-sm">Guardar informe</button>
        </div>
      )}
        ...
        <div className="space-y-2">
          {reports.map((r) => (
            <button
              ...
              className={`w-full text-left rounded-xl border p-4 transition-colors ${selected?.id === r.id ? "border-blue-500 bg-blue-950/30" : "border-slate-700 bg-slate-800/30 hover:border-slate-500"}`}
            >
