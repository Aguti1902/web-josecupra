  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {EXEC_KPIS.map((k) => (
          <div key={k.label} className="rounded-xl border border-slate-700 bg-slate-800/50 p-5">
            <p className="text-3xl font-black text-amber-400 stat-number">{k.value}</p>
            <p className="text-sm text-slate-400 mt-2">{k.label}</p>
          </div>
        ))}
      </div>
      ...
      <p className="text-xs text-slate-600 border border-slate-800 rounded-lg p-3">MAQUETA — KPIs y resumen con datos de ejemplo.</p>
    </div>
  );
}
