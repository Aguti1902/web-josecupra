...
import { DEPRO_ACCENT } from "@/lib/club-config";
    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100" : "bg-white"}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3">
          ...
          <img src="/logo-nexgent.png" alt="NexGent" className="hidden sm:block h-6 w-auto object-contain" />
        </Link>
        ...
        <div className="hidden sm:flex items-center gap-2">
          <Link href="/presentacion" className="text-xs font-bold text-gray-600 hover:text-gray-900 px-3 py-2">
            Diapositivas
          </Link>
          <Link href="/app/inicio" className="flex items-center gap-1.5 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors" style={{ backgroundColor: DEPRO_ACCENT }}>
      ...
      {menuOpen && (
        <div className="lg:hidden border-t border-gray-100 px-4 py-3 bg-white space-y-1">
          ...
          <Link href="/presentacion" className="block py-2.5 text-sm font-bold text-depro-blue">Ver diapositivas</Link>
          <Link href="/app/inicio" className="block py-2.5 text-sm font-bold text-depro-blue">Ver demo</Link>
        </div>
      )}
    </header>
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 md:py-16">
        <div className="flex flex-col items-center text-center max-w-2xl mx-auto pb-8 border-b border-gray-200">
          <div className="flex items-center justify-center gap-6 mb-6">
            <img src="/logo-nexgent.png" alt="NexGent" className="h-14 md:h-20 w-auto object-contain" />
            <span className="text-2xl text-gray-300 font-light">×</span>
            <img src="/logo.png" alt="DEPRO" className="h-10 md:h-14 w-auto object-contain" />
          </div>
