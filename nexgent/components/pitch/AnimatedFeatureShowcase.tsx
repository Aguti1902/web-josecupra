/**
 * Mini demos animados (efecto GIF) para el pitch — estilo us-clubs
 */
...
function DemoPanel({ type, step }: { type: string; step: number }) {
        ...
        <div className="flex gap-1.5 items-end h-24 mb-2">
          {[40, 65, step >= 2 ? 89 : 30, 55].map((h, i) => (
            <div key={i} className="flex-1 bg-gray-100 rounded-t relative overflow-hidden">
              <div className="absolute bottom-0 w-full rounded-t transition-all duration-700" style={{ height: `${h}%`, backgroundColor: i === 2 ? "#F59E0B" : accent }} />
            </div>
          ))}
        </div>
    ...
    return (
      <BrowserChrome title="chat staff">
        <div className="space-y-2">
          {(step >= 0 ? ["Sesión técnica mañana — intensidad media", "Militão Fase 2 readaptación"] : []).map((m, i) => (
