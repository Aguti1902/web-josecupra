  ...
  return (
    ...
    >
      <div className="relative z-10 p-6 flex flex-col sm:flex-row sm:items-center gap-4">
        <img
          src={PALMEIRAS.logo}
          alt={PALMEIRAS.shortName}
          className="w-16 h-16 rounded-xl object-contain bg-white p-1.5 flex-shrink-0 shadow-lg"
        />
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold uppercase tracking-wider mb-0.5" style={{ color: mutedColor }}>
            {new Date().toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long" })}
          </p>
          <h2 className="text-2xl font-black truncate" style={{ color: textColor }}>
            {PALMEIRAS.name}
          </h2>
          <p className="text-sm mt-0.5" style={{ color: mutedColor }}>
            Avante Palestra · {PALMEIRAS.city}
          </p>
        ...
        <div
          ...
          style={{ backgroundColor: secondary, color: "#006437" }}
        >
          Semana 24
        </div>
      </div>
