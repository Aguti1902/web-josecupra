...
const COMPARE_VALUES = [
  { depro: true, sheets: false, generic: false },
  { depro: true, sheets: "partial", generic: false },
  { depro: true, sheets: false, generic: "addon" },
  { depro: true, sheets: false, generic: false },
  { depro: true, sheets: false, generic: false },
  { depro: true, sheets: false, generic: "limited" },
  { depro: true, sheets: false, generic: false },
  { depro: true, sheets: false, generic: "limited" },
  { depro: true, sheets: true, generic: "limited" },
];

const ROADMAP_STATUS = ["statusDone", "statusCurrent", "statusNext", "statusFuture"];
const ROADMAP_COLORS = ["#22C55E", "#0A36F7", "#F59E0B", "#9CA3AF"];
...
function CompareCell({ value, t }) {
  ...
  return <span className="text-xs font-bold text-amber-600">{label}</span>;
}
...
export default function NexGentPitchLanding() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section id="roadmap" className="py-20 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="max-w-2xl mb-12">
          ...
          <div className="relative">
            <div className="hidden lg:block absolute left-[19px] top-8 bottom-8 w-0.5 bg-gray-200" />
            <div className="space-y-6">
              {Array.isArray(roadmapPhases) && roadmapPhases.map((phase, i) => (
                <div key={phase.phase} className="relative flex gap-6 lg:gap-8">
                  <div className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-black z-10 shadow-md" style={{ backgroundColor: ROADMAP_COLORS[i] }}>
                    {i + 1}
                  </div>
                  <div className="flex-1 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: ROADMAP_COLORS[i] + "18", color: ROADMAP_COLORS[i] }}>
                        {t(`roadmap.${ROADMAP_STATUS[i]}`)}
                      </span>
                    </div>
                    <h3 className="font-black text-lg text-gray-900 mb-3">{phase.title}</h3>
      ...
      <section id="comparativa" className="py-20 border-b border-gray-100 bg-gray-50">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left p-4 font-bold text-gray-700">{t("compare.capability")}</th>
                  <th className="p-4 font-bold text-center" style={{ color: ACCENT }}>{t("compare.us")}</th>
                  <th className="p-4 font-bold text-center text-gray-500">{t("compare.sheets")}</th>
                  <th className="p-4 font-bold text-center text-gray-500">{t("compare.generic")}</th>
                </tr>
              </thead>
              <tbody>
                {Array.isArray(compareRows) && compareRows.map((label, i) => (
                  <tr key={label} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="p-4 text-center bg-blue-50/50"><CompareCell value={COMPARE_VALUES[i]?.depro} t={t} /></td>
        ...
        <section className="py-20 md:py-28">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
            <div className="flex items-center justify-center gap-4 mb-8">
              <img src={NEXGENT_LOGO} alt="NexGent" className="h-16 md:h-20 object-contain" />
              <span className="text-gray-300 text-2xl">×</span>
              <img src={DEPRO_LOGO} alt="DEPRO" className="h-12 md:h-16 object-contain" />
            </div>
